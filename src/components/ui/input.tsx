  ...
  ({ className, type, onChange, onBlur, ...props }, ref) => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if ((type === 'number' || props.inputMode === 'decimal') && originalValue.includes(',')) {
        ...
        // Important: Set back the display value with comma to maintain user's input format
        setTimeout(() => {
          if (e.target) {
    ...
    const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
      if ((type === 'number' || props.inputMode === 'decimal') && e.target.value) {
        ...
        // If it's a valid number, keep it processed internally
        if (!isNaN(parseFloat(value))) {
          // For internal processing, we use the value with dots
          const displayValue = e.target.value; // Keep the display value as user entered it
          ...
          // Call original onBlur if provided with processed value
          if (onBlur) onBlur(newEvent);
          
          // Restore display value for user
          setTimeout(() => {
            if (e.target) {
    ...
    return (
      <input
        type={type === 'number' ? 'text' : type} // Convert number inputs to text for better comma handling
        className={cn(
